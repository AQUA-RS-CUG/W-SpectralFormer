...
def get_s2_data_path(root_dir, cur_data):
    s2_path = None
    cur_data_dir = os.path.join(root_dir, cur_data)
    for filename in os.listdir(cur_data_dir):
        if filename[:2] == 'S2':
            s2_path = os.path.join(cur_data_dir, filename)
    return s2_path


def load_s2_image(root_dir, cur_data):
    s2_path = get_s2_data_path(root_dir, cur_data)
    if s2_path is not None:
        image = np.load(s2_path)
        return image
    else:
        raise FileNotFoundError(f"No S2 data found in {cur_data}")
