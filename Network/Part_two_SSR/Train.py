...
from Network.whole_work import W_SpectralFormer
from Physics_loss import LossQAA, Loss_SAM


def adjust_coefficient(epoch):
    if epoch < 5:
        a = 0.0
    else:
        a = 0.65
...
def test(opt, model, test_dates, IMAGE_SIZE, PATCH_SIZE):
    ...
    for cur_data in test_dates:
        ...
        if cur_day == 6:
            ...
            output_image_OLCI = np.zeros(images[0].shape)
            image_mask_OLCI = np.ones(images[0].shape)
            inf_mask_OLCI = np.where(images[0] > 1000)
            ...
            real_im_OLCI = images[0] * image_mask_OLCI
            real_im_MS2 = images[1] * image_mask
            real_output_OLCI = output_image_OLCI * image_mask_OLCI
            real_output = output_image_MS2 * image_mask

            for real_predict in [real_output_OLCI]:
                cur_result_OLCI = {}
                cur_result_OLCI['rmse'] = []
                cur_result_OLCI['ssim'] = []
                cur_result_OLCI['cc'] = []
                cur_result_OLCI['uiqi'] = []
                cur_result_OLCI['ergas'] = 0

                for i in range(14):
                    cur_result_OLCI['rmse'].append(rmse(real_im_OLCI[i], real_predict[i]))
                    cur_result_OLCI['ssim'].append(ssim(real_im_OLCI[i], real_predict[i], MAX=1.0)[0])
                    cur_result_OLCI['uiqi'].append(uiqi(real_im_OLCI[i], real_predict[i]))
                    cur_cc_OLCI = np.sum(
                        (real_im_OLCI[i] - np.mean(real_im_OLCI[i])) * (real_predict[i] - np.mean(real_predict[i]))) / \
                                  np.sqrt((np.sum(np.square(real_im_OLCI[i] - np.mean(real_im_OLCI[i])))) * np.sum(
                                      np.square(real_predict[i] - np.mean(real_predict[i]))) + 1e-100)
                    cur_result_OLCI['cc'].append(cur_cc_OLCI)
                    cur_result_OLCI['ergas'] += rmse(real_im_OLCI[i], real_predict[i]) ** 2 / (
                                np.mean(real_im_OLCI[i]) ** 2 + 1e-100)

                cur_result_OLCI['ergas'] = np.sqrt(cur_result_OLCI['ergas'] / 6.) * 6
                cur_result_OLCI['psnr'] = psnr(real_im_OLCI, real_predict, MAX=1.0)
                cur_im_OLCI = real_im_OLCI * 10000.
                cur_predict_OLCI = real_predict * 10000.
                cur_result_OLCI['sam'] = sam(cur_im_OLCI.transpose(1, 2, 0),
                                             cur_predict_OLCI.transpose(1, 2, 0)) * 180 / np.pi

                print('[%s] 2_RMSE: %.4f SSIM: %.4f UIQI: %.4f CC: %.4f ERGAS: %.4f SAM: %.4f PSNR: %.4f' % (
                    cur_data, np.mean(np.array(cur_result_OLCI['rmse'])),
                    np.mean(np.array(cur_result_OLCI['ssim'])), np.mean(np.array(cur_result_OLCI['uiqi'])),
                    np.mean(np.array(cur_result_OLCI['cc'])), cur_result_OLCI['ergas'], cur_result_OLCI['sam'],
                    cur_result_OLCI['psnr']))

                final_sam_OLCI = cur_result_OLCI['sam']
            ...
            for real_predict in [real_output]:
                ...
                cur_result_MS2['psnr'] = psnr(real_im_MS2, real_predict, MAX=1.0)
                ...
                print('[%s] 1_RMSE: %.4f SSIM: %.4f UIQI: %.4f CC: %.4f ERGAS: %.4f SAM: %.4f PSNR: %.4f' % (
...
def train(opt, train_dates, test_dates, IMAGE_SIZE, PATCH_SIZE):
    ...
    model = W_SpectralFormer()
    n_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print('There are %d trainable parameters for generator.' % n_params)
    model.to(device)

    cri_l1 = nn.L1Loss(reduction='mean').to(device)
    cri_sam = Loss_SAM().to(device)
    cri_QAA = LossQAA().to(device)
    ...
    for epoch in range(opt.num_epochs):
        ...
        for item, (OLCI, MSI, Mask) in enumerate(train_loader):
            t_start = timer()
            target = OLCI.to(device)
            input = MSI.to(device)
            Mask = Mask.to(device)  # water

            m1, m1_deg_SRF, m1_deg_PSF = model(input)
            optimizer.zero_grad()

            Spatial_loss = cri_l1(m1_deg_SRF, input)
            Spectral_loss = cri_l1(m1_deg_PSF, target)
            SAM_loss = 0.001 * cri_sam(m1_deg_PSF, target)
            a = adjust_coefficient(epoch)
            Physics_loss = cri_QAA(m1_deg_SRF * Mask, input * Mask)
            loss = Spatial_loss + Spectral_loss + SAM_loss + a * Physics_loss
        ...
        f = open(save_dir + 'QAA_loss.txt', 'a')
...
def main():
    ...
    parser.add_argument('--root_dir', default='L:\SSR\ALL\Root_dir_all', help='Datasets root directory')
    parser.add_argument('--train_dir', default='L:\SSR\ALL\Data_NPY_ALL', help='Datasets train directory')
    ...
    for dir_name in os.listdir(opt.root_dir):
        ...
        if cur_day not in [6, 8, 12, 16, 21, 24, 26, 29]:
